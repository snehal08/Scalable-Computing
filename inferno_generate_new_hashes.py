...
#print pot_lines[0]
#print pot_lines[0].split(' ')[1]
new = pot_file.readlines()
pot_only_hashes = []
pot_only_passwords = []
for cracked in new:
    cracked_sep = cracked.rstrip().split(':')
    print cracked_sep
    pot_only_hashes.append(cracked_sep[0])
    pot_only_passwords.append(cracked_sep[1])
    #print (cracked)
    #pwd = cracked.rstrip().split(' ')
    #print (pwd)
    #if str(pwd[0]+" ")in hashes:
    #index = hashes.index(str(pwd[0])+" ")
        #print (index)
        #print (hashes[index])
    #hashes[index] = hashes[index]+pwd[1]
#pot_only_hashes = [i.split(' ')[1] for i in pot_lines]
#print pot_only_hashes
